  ...
  return (
    <div className="w-full h-full flex flex-col justify-center relative bg-[var(--bg-primary)] p-4 md:p-20 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto flex flex-col h-full justify-center">
        ...
        <div className="grid grid-cols-2 gap-2 md:gap-8 px-0 md:px-0">
          {certsData.map((cert, i) => (
            <motion.div
              key={i}
              whileHover={{ scale: 1.02, y: -5 }}
              transition={{ duration: 0.3 }}
              className="flex flex-col items-center justify-center p-3 sm:p-4 md:p-10 border border-[var(--glass-border)] bg-[var(--glass-bg)] hover:bg-[var(--glass-bg)]/80 transition-colors group backdrop-blur-sm relative overflow-hidden"
              style={{
